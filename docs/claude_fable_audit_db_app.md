---
type: "Audit Prompt"
title: "Claude Fable: DBを使うアプリ向けセキュリティ・脆弱性・バグ監査"
description: "Claude Fable の深い推論で DB を使うアプリのセキュリティ・脆弱性・バグ・保守性を監査する貼り付け用プロンプト。"
tags: ["audit", "claude_fable", "db_app"]
status: "stable"
audit:
  tool: "claude_fable"
  target: "db_app"
...
Fable の深い推論（拡張思考・1M コンテキスト）を活かし、調査・修正は**単一エージェント**で行う監査。敵対的検証と完了条件（rubric）の採点だけは、独立コンテキストの verifier サブエージェントへ委託する（同一コンテキストでの self-critique は精度が落ちるため）。ultracode（並列ワークフロー）は使わない。DBを使うアプリを対象に、セキュリティ、脆弱性、バグ、保守性の問題を調査し、レポートと修正案を出力する（既定ではソースを書き換えない。スコープ指定時のみ現行機能を壊さない範囲で修正まで行う）。ビルド、コンパイル、コミット、抜本的なアーキテクチャ改修は禁止。

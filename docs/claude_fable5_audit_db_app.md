# Claude Fable 5: DBを使うアプリ向けセキュリティ・脆弱性・バグ監査

Claude Code で Fable 5 モデル（`/model claude-fable-5`）を使って実行するための監査プロンプト。

Fable 5 の深い推論（拡張思考・1M コンテキスト）を活かした**単一エージェント**監査。ultracode（並列ワークフロー）は使わない。DBを使うアプリを対象に、セキュリティ、脆弱性、バグ、保守性の問題を調査し、現行機能を壊さない範囲で修正する。ビルド、コンパイル、コミット、抜本的なアーキテクチャ改修は禁止。

```text
このリポジトリを Fable 5 の深い推論で監査してください。
...
この goal は Fable 5 の深い推論を活かした単一エージェント監査として設計されています。観点ごとに順に調査し、各 finding を構造化して扱ってください。拡張思考を活用し、根拠が曖昧なまま finding を確定しないでください。
...
以下の観点を順に調査してください。Fable 5 の拡張思考を活かし、各観点を深く・根拠を持って調査すること。
...
- DBを使う前提と、DB構成 / 永続化方式が plan md に記録されている
- 対象コードをセキュリティ、脆弱性、バグ、保守性、DB / データ整合性の観点で一通り調査している
- finding を敵対的に検証し、確定したものだけ修正対象にしている
- 見つけた高優先度の問題を、現行機能を壊さない最小修正で可能な範囲で修正している
- DBスキーマ大幅変更、DBエンジン変更、ORM全面移行、大規模 migration、データ移行は行っていない
- DB migration の実適用、本番DB接続、本番DB変更は行っていない

...
title: "サーバー診断プロンプト 共通不変条件（正本）"
description: "サーバー診断プロンプト全体で共有する完全 read-only の安全境界・診断・報告形式の不変条件を定義する正本。"
...
- 非 repo の server 監査は、開始前に report owner となる private 管理 repo を明示する。cwd がその repo でない場合や owner が未指定の場合は保存先を推測せず、実接続前に確認して止める。ホスト名・接続情報は必要最小限だけ report に記載し、秘密値は転記しない。
...
- **plan md**: 監査対象 repo の `docs/local/plan_<audit-topic>.md`。プロジェクトの命名ルールがあれば優先し、作業中ずっと更新する。
- **結果報告 md**: report owner である監査対象 repo の既定保存先
  `docs/ai-audit-prompts/report_audit_<topic>_<YYYY-MM-DD>.md` に作成する。起動時に
  `保存先=<repo 相対パス>` が明示された場合だけ report の保存先を変更する。対策提言の一覧（重大度順）を含め、
  frontmatter は `type: audit-report`、`status: draft|stable`、`tags`、`owner`、`related`、
  `last_reviewed`、`docsweep_policy: never_archive` とする。`docsweep_state` / `due` は付けない。
...
途中でセッションが切れた場合に「plan md だけ残って結果報告 md は空」となる事故を防ぐため、結果報告 md は最後にまとめて書かず、診断の進捗に合わせて逐次ドラフトを更新する。

- 初期準備フェーズで、解決済み保存先に結果報告 md の空テンプレを作成する。既定保存先
  ...
  の場合は既定保存先へ切り替えてよいかを確認し、無断で fallback しない。途中停止時は `status: draft` のまま未完了箇所を明記し、完了時だけ `stable` にする。
  非対話で未存在の通常フォルダを作成する場合は、Git 管理方針（`ignore` / `track`）を別途明示し、
  未指定なら作成せず停止する。
- finding が確定するたびに、plan md と同時に結果報告 md にも反映する（提言一覧・スコア・適用時の注意を追記または上書き）。同じ内容を plan md と結果報告 md の 2 箇所に書く重複は許容する（途中停止に備えた冗長化が目的）。
- 各フェーズ終端（診断・敵対的検証・提言）で結果報告 md のスコアと提言一覧を最新化する。

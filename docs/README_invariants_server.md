...
- **結果報告 md**: plan md とは別に作成。対策提言の一覧（重大度順）を含める。**初期準備フェーズで plan md と同じ場所に空テンプレ（総合評価ヘッダー骨格 + セクション見出し）として先に作成し、作って終わりにせず作業中ずっと更新する**（最後にまとめて書かない。詳細は次節「結果報告 md の逐次ドラフト運用」）。最終報告はユーザーが使用している言語で行う（指定がなければ日本語）。
...
- 初期準備フェーズで、plan md と同じディレクトリに結果報告 md を空テンプレで作成する（総合評価ヘッダー骨格 / 対象ホスト・役割推定 / 実施した診断 / 対策提言一覧（重大度順）/ 判断待ち事項 / 実行できなかった検査と理由 / その他「最終報告」に列挙される項目のセクション見出しを並べた骨格）。

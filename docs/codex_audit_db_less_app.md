---
type: "Audit Prompt"
title: "Codex Goal: DBを使わないアプリ向けセキュリティ・脆弱性・バグ修正"
description: "Codex CLI で DB を使わないアプリのセキュリティ・脆弱性・バグ・保守性を監査し、必要に応じて修正する貼り付け用プロンプト。"
tags: ["audit", "codex", "db_less_app"]
status: "stable"
audit:
  tool: "codex"
  target: "db_less_app"
...
DBを使わないアプリを対象に、セキュリティ、脆弱性、バグ、保守性の問題を調査し、レポートと修正案を出力する（既定ではソースを書き換えない。スコープ指定時のみ現行機能を壊さない範囲で修正まで行う）。ビルド、コンパイル、コミット、抜本的なアーキテクチャ改修は禁止。

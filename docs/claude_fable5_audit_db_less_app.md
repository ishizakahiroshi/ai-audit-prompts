# Claude Fable 5: DBを使わないアプリ向けセキュリティ・脆弱性・バグ監査

Claude Code で Fable 5 モデル（`/model claude-fable-5`）を使って実行するための監査プロンプト。

Fable 5 の深い推論（拡張思考・1M コンテキスト）を活かした**単一エージェント**監査。ultracode（並列ワークフロー）は使わない。DBを使わないアプリを対象に、セキュリティ、脆弱性、バグ、保守性の問題を調査し、現行機能を壊さない範囲で修正する。ビルド、コンパイル、コミット、抜本的なアーキテクチャ改修は禁止。
...
- DB前提で考えない / SQL・ORM・migration の観点に寄せない
- 変更前に既存パターンを読む
- 修正したら必ず再検証する
- 現行機能を壊す可能性のある変更は避ける
- 不具合修正に集中する

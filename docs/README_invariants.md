...
- **結果報告 md**: plan md とは別に作成。**初期準備フェーズで plan md と同じ場所に空テンプレ（総合評価ヘッダー骨格 + 対応状況サマリ + 対応一覧 + C 詳細 + セクション見出し）として先に作成し、作って終わりにせず作業中ずっと更新する**（最後にまとめて書かない。詳細は次節「結果報告 md の逐次ドラフト運用」）。結果報告 md は監査後対応の現在値を確認する正本とし、最終報告はユーザーが使用している言語で行う（言語指定がなければ日本語）。
...
- 初期準備フェーズで、plan md と同じディレクトリに結果報告 md を空テンプレで作成する（総合評価ヘッダー骨格 / 対応状況サマリ / 対応一覧（概要） / C 詳細 / 今回の実装内容 / 確定 finding 一覧 / 対処手順（実務）/ 未完了項目 / 判断待ち事項 / 抜本改修の提言 / その他「最終報告の形式」に列挙される項目のセクション見出しを並べた骨格）。
...
結果報告 md は監査結果の説明だけでなく、監査後対応を追跡する主文書として扱う。総合評価ヘッダーの直後に `## 対応状況サマリ` と `## 対応一覧（概要）` を置き、その下に対応 C の詳細を置く。
...
- audit 用 plan md は調査ログ・確認済みルール・敵対的検証を記録し、対応状況の正本にはしない。結果報告 md を開けば、監査後に次に行う C が分かる状態にする。
